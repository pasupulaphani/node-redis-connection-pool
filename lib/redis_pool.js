...
const selectDB = function (client, db) {
  return new Promise((resolve, reject) => {
    client.select(db, function (err) {
      ...
      this.logger.info("DB selected: ", db);
      resolve(client);
    });
  });
};
    ...
    create: () => {
      ...
      return retry(function () {
        ...
        if (createAttempts > 3) {
          ...
          this.logger.error("Max conn createAttempts reached: %s, resolving to error:", createAttempts, err);
        ...
        return create(this.redisOptions);
      }, {
        max: 10,
        name: "factory.create",
        report: debug
...
RedisPool.prototype.sendCommand = function (commandName, commandArgs) {
  return this.pool.acquire()
    .then(conn => conn.send_command(commandName, commandArgs)
        .then(result => {
